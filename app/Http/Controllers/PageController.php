...
{
    public function parsePageForm()
    {
        //Возвращает шаблон с полем для добавления отслеживаемого продукта
        return view('welcome');
    }
    ...
    public function parsePage(Request $request)
    {
        ...
        // Записывает в базу данных продукт, продавцов, которые его продают, и текущие цены
        ...
        $product = Product::firstOrCreate(['name'=> $productId], ['url'=>$request->input('url')]);

        if (!$productId) {
            return back()->withErrors(['error' => 'Не удалось извлечь ID продукта из URL']);
        }

        $cacheKey = 'product_prices_' . $productId;
        $productPrices = Cache::remember($cacheKey, 60, function() use ($url, $productId) {
            return $this->fetchPrices($url, $productId);
        });
        ...
        return view('welcome', ['price' => $productPrices]);
    }

    private function fetchPrices(string $url, string $productId)
    {
        // Отправляет запрос на получение цен, собирает их в массив и возвращает
        $headers = [
        ...
        $data = [
            ...
            'id' => $this->extractProductIdFromUrl($url),
            'merchantUID' => '',
            'product' => [
                'brand' => 'Leadbros',
                'categoryCodes' => [
    ...
    {
        //Извлекает из URL ID товара на каспи
        $pattern = '/-([0-9]+)(?:\/\?c=|$)/';
    ...
    function extractCityId(string $url) {
        //Извлекает из URL ID города на каспи
